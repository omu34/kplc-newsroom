<div class="bg-white dark:bg-gray-950 py-12 sm:py-16">
    @php
        use Carbon\Carbon;
        $mainPages = App\Models\MainPage::orderBy('created_at', 'asc')->get();
    @endphp
                            ...
                            <article class="relative isolate flex flex-col transition-transform transform hover:scale-105 justify-end overflow-hidden rounded-2xl bg-gray-900 px-4 pb-8 pt-44 sm:pt-60 lg:pt-48">
                                @if ($post->link)
                                    <a href="{{ $post->link }}" target="_blank" class="absolute inset-0 z-10">
                                        <img src="{{ asset('storage/' . $post->file) }}" alt="" class="absolute inset-0 -z-10 h-full w-full object-cover">
                                        <div class="absolute inset-0 -z-10 bg-gradient-to-t from-black via-gray-900/50"></div>
                                        <div class="absolute inset-0 -z-10 rounded-2xl ring-1 ring-inset ring-gray-900/10"></div>
                                        <img src="{{ asset('storage/' . $post->file) }}" class="absolute text-yellow top-2/3 mb-8 left-1/2 transform -translate-x-1/2 -translate-y-28 h-12 w-12 fill-white" alt="">
                                    </a>
                                    <div class="flex flex-wrap items-center gap-y-1 overflow-hidden text-base leading-6 text-white">
                                        <time datetime="{{ $post->day }}" class="text-white mr-6">
                                            @if ($post->day)
...
{{--  <div class="bg-white dark:bg-gray-950 py-12 sm:py-16">
    <div class="mx-auto lg:px-0 px-4 max-w-7xl">
        <div class="bg-white dark:bg-gray-950 py-12 sm:py-16">
            <div class="mx-auto flex flex-col items-center sm:flex-row sm:justify-between mb-8">
                <h2 class="text-3xl font-bold tracking-tight lg:pb-0 pb-5 text-gray-900 dark:text-white sm:text-3xl">
                    {{ $posts->first()->main_page_name12 }}
                </h2>
                <button wire:click="toggleShowPosts"
                    class="px-4 py-2 rounded-md font-medium text-base leading-6 shadow-sm
                    @if ($showPosts) bg-blue-500 text-white @else bg-gray-200 text-gray-700 hover:bg-gray-300 @endif">
                    @if ($showPosts)
                        On
                    @else
                        Off
            ...
            @if ($showPosts)
                @include('livewire.featured-news-content', ['posts' => $posts])
            @endif
            @include('livewire.featured-news', ['posts' => $posts, 'showPosts' => $showPosts])
