<div>
    <div class="grid grid-cols-1 gap-8 sm:mt-4 lg:mx-0 lg:max-w-none lg:grid-cols-2">
        @foreach ($blogs as $blog)
            <article class="relative flex flex-col transition-transform transform hover:scale-105 justify-end overflow-hidden rounded-2xl bg-gray-900 px-4 pb-8 pt-44 sm:pt-60 lg:pt-48">
                <img src="{{ asset('storage/' . $blog->image_path) }}" alt="" class="absolute inset-0 -z-10 h-full w-full object-cover">
                <div class="absolute inset-0 -z-10 bg-gradient-to-t from-black via-gray-900/50"></div>
                <div class="absolute inset-0 -z-10 rounded-2xl ring-1 ring-inset ring-gray-900/10"></div>
                <img src="{{ asset('storage/' . $blog->video_path) }}" class="absolute top-2/3 mb-8 left-1/2 transform -translate-x-1/2 -translate-y-28 h-12 w-12 fill-white" alt="">

                <!-- Metadata Section -->
                <div class="flex flex-wrap items-center gap-y-1 overflow-hidden text-base leading-6 text-white">
                    <time datetime="{{ $blog->day }}" class="text-white mr-6">
                        {{ $blog->date_updated ? \Carbon\Carbon::parse($blog->day)->format('M d, Y') : 'N/A' }}
                    </time>
                    <div class="-ml-4 flex items-center gap-x-2">
                        <svg viewBox="0 0 2 2" class="-ml-0.5 h-0.5 w-0.5 flex-none fill-white">
                            <circle cx="1" cy="1" r="1" />
                        </svg>
                        <div class="flex text-white">views {{ $blog->views }}</div>
                    </div>
                </div>
                ...
                <div class="flex mt-2 flex-wrap items-center gap-y-1 overflow-hidden text-base leading-6 text-gray-300">
                    ...
                    <div class="-ml-4 flex items-center gap-x-2">
                        <div class="flex text-white gap-x-2">likes {{ $blog->likes }}</div>
                    </div>
                </div>
            </article>
        @endforeach
