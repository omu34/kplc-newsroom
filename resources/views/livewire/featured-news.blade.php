    ...
    @forelse ($mainPages->take(4) as $page)
        <div class="mx-auto lg:px-0 px-4 max-w-7xl">
            <div class="mx-auto">
                ...
                @php
                    $blogs = App\Models\Blog::orderBy('created_at', 'asc')->take(4)->get();
                @endphp
                ...
                @foreach ($blogs as $index => $blog)
                    <article
                        ...
                        <img src="{{ asset('storage/' . $blog->another_image_path) }}"
...
{{--  @foreach (App\Models\Blog::orderBy('created_at', 'desc')->limit(4)->get() as $index => $blogs)
  @if ($blogs)
    <a href="{{ $blogs->link }}" class="flex text-base items-center text-gray-800 font-bold hover:text-gray-900 py-2 px-3 relative">
      @if ($blogs->video_path)
        <video src="{{ asset('storage/' . $blogs->video_path) }}" class="w-12 h-12 mr-3 absolute top-0 left-0 object-cover opacity-0 hover:opacity-100" controls autoplay muted loop></video>
      @endif
      <div class="flex flex-col ml-16">
        <span class="text-base font-bold">{{ $blogs->name }}</span>
        <div class="text-sm text-gray-600">
          <span class="mr-2">
            Updated:
            @if ($blogs->date_updated)
              {{ \Carbon\Carbon::parse($blogs->date_updated)->format('M d, Y') }}
            @else
              N/A
            @endif
          </span>
          <span class="mr-2">Views: {{ $blogs->views }}</span>
          <span>Likes: {{ $blogs->likes }}</span>
        </div>
        @if ($blogs->description)
          <p class="mt-2 text-sm text-gray-600">{{ $blogs->description }}</p>
        @endif
      </div>
    </a>
  @endif
